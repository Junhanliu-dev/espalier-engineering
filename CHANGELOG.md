...
- **`skills/espalier-init/SKILL.md` + `references/discovery-checklist.md`** — Phase 1 gains a security-surface scout (1.11 — entry points, identity/ownership/validation patterns, project sensitive fields → `DISCOVERY.security`); Phase 2 writes the three new files. **`scripts/bootstrap-espalier.sh`** wires them (mkdir + 3 symlinks) and grows to **32 validation checks** (added: security rule, agent, skill).
- **`scripts/migrate-v0.8.2-to-v0.9.0.sh`** (new) — idempotent target-repo upgrade: creates the 3 new files (project-name + tools-mode substitution), `bootstrap --force` refreshes the pure-copy pipeline files + symlinks + validates, surgical appends add the security sections to the per-project coder/reviewer/testing files, a surgical insert adds the scan to the push gate. Backs up customised pipeline files (`<file>.pre-v0.9.0.bak`). `--dry-run` / `--yes` / `--plugin-dir=`.
- **`skills/espalier-migrate/SKILL.md`** — the auto-detect chain gains the v0.8.2 → v0.9.0 step (a tenth migration; the Stage 4 panel + `harness-security` agent absent ⇒ needed).
- **`eval/security/`** (new, dev/QA — not shipped) — an eval harness for `harness-security`, mirroring `eval/grill`: runner + rubric + 7 golden fixtures (5 vuln across the five axes + mass-assignment + queue-consumer; 2 clean for self-noop + false-positive precision). Gates on catch-rate ≥ 0.90 with zero false positives, so a prompt edit that regresses the auditor's judgment fails closed.

Additive to every existing install — new pipeline runs get the audit, and the auditor self-noops on non-security changes so the cost lands only where it matters. See [docs/migrating-v0.8-to-v0.9.md](./docs/migrating-v0.8-to-v0.9.md).

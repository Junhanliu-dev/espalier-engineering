...
false_positive_watch:
  ...
  - "planId is validated against an allow-list (PLANS) and price is recomputed from the plan, not the body — do NOT flag money"
shadow: true
---
const db = require('./db');
...
async function changePlan(req, res) {
  try {
    ...
    if (priceCents == null) return res.status(422).json({ error: 'unknown plan' });

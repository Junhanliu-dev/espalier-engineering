...
#      abuse-test coupling), symlinks the 3 new files, runs the 32-check validation.
...
# Already-v0.9.0 detector — ALL FIVE artifacts must be present, not just the two
...
if [ "$PANEL_DONE" = yes ] && [ "$AGENT_DONE" = yes ] && [ "$CODER_DONE" = yes ] \
   && [ "$REVIEWER_DONE" = yes ] && [ "$GATE_DONE" = yes ]; then
  echo "Already on v0.9.0 (all security artifacts present)."
  echo "Nothing to do."
  exit 0
fi
...
Plan:
  ...
  2. Create 3 new files (project="$PROJECT_NAME", tools=$AGENT_TOOLS):
       $SECRULE
       $SECAGENT
       $SECSKILL
  3. bootstrap-espalier.sh --force  (merge-decision=$DECISION, doctor-cadence=$DOCTOR_CADENCE)
       → Stage 4 panel + abuse-test coupling; symlinks new files; 32-check validation
  4. Append security sections to $CODER, $REVIEWER, $TESTING
  5. Insert the secret/dependency scan into $GATE
  6. Verify everything landed
...
log "Step 4: append security sections"

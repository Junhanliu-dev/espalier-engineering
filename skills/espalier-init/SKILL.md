...
On a **bare folder** (no manifest, no source files) the Entry Gate (Phase −1)
routes to the greenfield flow — same flow `/espalier-new` enters directly.
...
└── references/                    # deep-dive content read on demand
    ...
    ├── greenfield.md              # Phase −1 bare-repo flow (grill → scaffold → verify → converge)
    └── greenfield/                # per-track deep dives, read after track chosen
        ├── frontend.md  backend.md  fullstack.md  mobile.md
        ├── cms.md  microservices.md  cli-library.md  desktop.md
        └── deploy-targets.md      # guided first-deploy flows
```
...
This applies to **all** skill folders generated by this skill: espalier-coding, espalier-review, espalier-testing, espalier-requirements, espalier (main), espalier-fix, espalier-prune, espalier-doctor, and any additional ones added later.
...
## Phase −1: Entry Gate (bare-repo check)

Before Phase 0, classify the repo. Run:
...
- **Any output → NOT bare.** Continue to Phase 0 below. (This is the normal
  case; behavior is identical to pre-greenfield versions.)
- **Empty output → bare** (no package manifest, no source files —
  README/LICENSE/.git/docs/editor config don't count). A bare repo has no
  conventions to discover. Read `references/greenfield.md` and follow it:
  adaptive grill → scaffold a production-grade stack → verification gate →
  converge back into Phase 0 → 1 → 2 → 3 below. Requires a TTY; greenfield
  fails fast on non-interactive runs.
...
- **Stage 11:** run 28 validation checks (27 in parallel, #25 serial for its tier table); print sorted output; non-zero exit if any failed.

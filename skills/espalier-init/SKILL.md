...
This applies to **all** skill folders generated by this skill: espalier-coding, espalier-review, espalier-security, espalier-testing, espalier-requirements, espalier-grill, espalier (main), espalier-fix, espalier-prune, espalier-doctor, espalier-ask, espalier-audit, espalier-map, and any additional ones added later.
...
- **Stage 3:** `cp` pure-copy templates → `espalier/pipeline.md`, `espalier/skills/{espalier,espalier-fix,espalier-requirements,espalier-grill,espalier-prune,espalier-doctor,espalier-ask,espalier-audit,espalier-map}/SKILL.md`. (No conflict with Phase 2 outputs — different files.)
...
- **Stage 11:** run the validation checks — 50 when only claude is targeted, 55 with codex, 60 with copilot (all but #25 in parallel, #25 serial for its tier table; a greenfield Pass 1 renders the Phase-2-artifact checks as pending-skips); print sorted output; non-zero exit if any failed.

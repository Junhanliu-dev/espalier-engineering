...
> **v0.4.0+ note:** Phase 11 runs via `scripts/bootstrap-espalier.sh` (Stage 11 of that script — 24 checks executed in parallel). Normal flow invokes this automatically. Manual usage: `bash scripts/bootstrap-espalier.sh --validate-only --plugin-dir=...` to re-run only the validation block (e.g., after manual file edits). The per-check table below describes what each check verifies and is retained as the source of truth for the check definitions.
...
| 20 | If decision=installed → hook live | `[ "$(cat espalier/.merge-hook-decision)" = "installed" ] && grep -qE "(ESPALIER\|HARNESS)_BACKLINK_HOOK" .git/hooks/post-merge .husky/post-merge 2>/dev/null` | Match found (or N/A if decision ≠ installed) |

...
version. Up to THIRTY-TWO migrations may apply, always in this order:
...
25. **v0.17.0 → v0.18.0** — map-lane release. New files: the `espalier-map`
   lane skill (multi-session planning — decision maps under `espalier/maps/`)
   + symlinks on every platform in `espalier/.platforms`, and the
   `map-guard.sh` plan-don't-do PreToolUse hook. Refreshes `pipeline.md`, the
   espalier + espalier-grill SKILL files (grill gains `mode=decision`) with
   backup-on-diff (`<file>.pre-v0.18.bak`). Wiring: settings.json gains the
   map-guard PreToolUse entry (additive), `.codex/config.toml` the
   marker-guarded `ESPALIER MAP GUARD v1` block, the copilot gates json a
   targeted map-guard insert (never clobbered). Config: appends
   `max-open-tickets: 9`. One grep-guarded `/espalier-map` line lands in each
   platform instruction file's existing `## Espalier` section. Mechanical:
   `scripts/migrate-v0.17.0-to-v0.18.0.sh`.
26. **v0.18.0 → v0.19.0** — run-lane release. New files: the `espalier-maprun`
   lane skill (batch executor for a CLEARED map — interactive master +
   headless stage-1–6 workers in isolated push-blocked worktrees) + symlinks
   on every platform in `espalier/.platforms`, and the run engine
   `espalier/hooks/maprun.py` + `maprun-{dispatch,merge,integration,verify}.sh`.
   Engine files are WRITE-IF-ABSENT — a repo already carrying a
   locally-adapted engine keeps it. Refreshes `pipeline.md` and the
   espalier-map SKILL (handoff now names the batch executor) with
   backup-on-diff (`<file>.pre-v0.19.bak`). One grep-guarded `/espalier-maprun`
...
28. **v0.20.0 → v0.21.0** — pipeline-speed release. Same gates, round caps,
   and review contract; the run just reads (and waits) less. Refreshes 4
   pure-copy files from templates with backup-on-diff (`<file>.pre-v0.21.bak`):
   `pipeline.md` (delta-scope doctrine, parallel-dispatch constraint,
   pre-authorized push checkpoint) and the espalier / espalier-fix /
   espalier-grill SKILLs (context-pack assembly at Stage 3 entry, spawn
   prompts carrying it, push-target question at the approval gate, grill
   light-tier independent-question batching). Anchored inserts into the 3
   per-project agent files (substituted at init, so never overwritten): the
   coder's context-pack step 0, the reviewer's step 0 + delta read-scope
   paragraph, the security agent's step 0 + delta-mode paragraph — a
   customised agent file missing its anchor is skipped with a
   `espalier/.migrations-skipped` record and manual instructions, never
   mangled. No config keys, no hook wiring, no new symlinks. Mechanical:
   `scripts/migrate-v0.20.0-to-v0.21.0.sh`.
29. **v0.21.0 → v0.21.1** — comment-brevity polish patch (field feedback:
   generated comments ran too long). Anchored edits to 3 per-project files:
   the coder gains a "Comments: SHORT, clear, and few" constraint bullet,
   `coding-standards.md` a "Keep comments SHORT" bullet under Comments &
   Docstrings, and the reviewer's Readability `comments:` tag now also flags
   an OVERLONG comment. A documented project convention requiring fuller
   docstrings outranks the new default. Customised files are skipped with a
   `.migrations-skipped` record, never mangled; backups at
   `<file>.pre-v0.21.1.bak`. Mechanical: `scripts/migrate-v0.21.0-to-v0.21.1.sh`.
30. **v0.21.1 → v0.22.0** — second pipeline-speed release. Same gates,
   sentinels, round caps, certificates, and human checkpoints; the changes
   are dispatch-order, read-scope, and wait-scheduling only. Refreshes 4
   pure-copy files with backup-on-diff (`<file>.pre-v0.22.bak`):
   `pipeline.md` (speculative Stage 5 trigger, Stage 6 delta note, CI wait
   protocol, Deploy-Target checkpoint + unattended posture) and the
   espalier / espalier-fix SKILLs (3-agent round-1 dispatch, Stage 4→5
   handoff with quarantine-on-FAIL, pre-flight fold, deploy pre-auth
   question, concurrent build+lint gate, one-invocation Stage 7
   bookkeeping) plus `espalier/hooks/espalier-stats.sh` (stage-duration
   report). Anchored edits into the 4 SUBSTITUTED per-project files: the
   ...
   absent = serial (init-time opt-in only). Mechanical:
   `scripts/migrate-v0.21.1-to-v0.22.0.sh`.
31. **v0.22.0 → v0.22.1** — comment-diet polish patch (field feedback:
   agents still write too MANY comment lines, not just long ones). Anchored
   edits to 3 per-project files: the coder's Comments constraint becomes
   default-NO-comment with a delete-before-reporting re-scan (no banners,
   no signature doc-blocks, no narration), `coding-standards.md` gains a
   "Default to NO comment" bullet, and the reviewer's Readability
   `comments:` tag now also flags EXCESS comments naming the exact lines
   to delete. A documented project convention mandating fuller docs still
   outranks the default. Customised files are skipped with a
   `.migrations-skipped` record; backups at `<file>.pre-v0.22.1.bak`.
   Mechanical: `scripts/migrate-v0.22.0-to-v0.22.1.sh`.
32. **v0.22.1 → v0.23.0** — round-economy release (the Stage 3/5 fold). Same
   gates, sentinels, caps, certificates, and human checkpoints; the dispatch
   restructures so tests ride the coder: under `test-mode: folded` (the
   default) the Stage 3 exit gate runs build+lint+scoped tests every coder
   return, the 2-agent Stage 4 panel reviews code AND tests in one verdict,
   and Stages 5/6 are the security-contract phase (contract spawn → delta
   review) or SKIPPED rows — stage numbers stay 3→4→5→6→7 for the push
   gate, stats, and maprun parsers. Contract-FAIL fixes touching non-test
   files route back to a FULL panel round. Fix lane: regression
   verification joins the Stage 3 exit gate (`REGRESSION_VERIFIED_SCOPE`
   skip hash, last-line-wins); scope detectors count non-test files only.
   Findings digest: FAIL rounds snapshot bracketed finding lines; charted
   changes write `espalier/maps/{slug}/findings/` files (Completion, or
   ...
   `test-mode` absent = folded, legacy `speculative-tests` honored at
   read time (`off` → serial). Mechanical:
   `scripts/migrate-v0.22.1-to-v0.23.0.sh`.

Your job: detect which one(s) apply, locate the scripts, preview, get
confirmation, apply in order. A v0.1.x install needs ALL THIRTY-TWO; a v0.3.x
install needs the last thirty-one; a v0.4.x install needs the last thirty; a
v0.5.0–v0.5.2 install needs the v0.5.3 patch then v0.6 … v0.23.0; a
v0.5.3–v0.5.x install needs v0.6 … v0.23.0; a v0.6.x install needs
v0.7 … v0.23.0; a v0.7.x install needs v0.8 … v0.23.0; a v0.8.0 install needs
v0.8.1 … v0.23.0; a v0.8.1 install needs v0.8.2 … v0.23.0; a v0.8.2 install
needs v0.9.0 … v0.23.0; a v0.9.0 install needs v0.9.1 … v0.23.0; a v0.9.1
install needs v0.9.2 … v0.23.0; a v0.9.2 install needs v0.9.3, v0.9.4,
v0.10.0, v0.11.0, v0.12.0, v0.13.0, v0.13.1, v0.13.2, v0.14.0, v0.15.0, v0.16.0, v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.9.3 install
needs v0.9.4, v0.10.0, v0.11.0, v0.12.0, v0.13.0, v0.13.1, v0.13.2, v0.14.0, v0.15.0, v0.16.0, v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a
v0.9.4, v0.9.5, or v0.9.6 install needs v0.10.0, v0.11.0, v0.12.0, v0.13.0,
v0.13.1, v0.13.2, v0.14.0, v0.15.0, v0.16.0, v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.10.0 install needs v0.11.0, v0.12.0, v0.13.0,
v0.13.1, v0.13.2, v0.14.0, v0.15.0, v0.16.0, v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.11.0 install needs v0.12.0, v0.13.0, v0.13.1,
v0.13.2, v0.14.0, v0.15.0, v0.16.0, v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.12.0 install needs v0.13.0, v0.13.1, v0.13.2, v0.14.0, v0.15.0,
v0.16.0, v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.13.0 install needs v0.13.1, v0.13.2, v0.14.0, v0.15.0, v0.16.0, v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.13.1
install needs v0.13.2, v0.14.0, v0.15.0, v0.16.0, v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.13.2 install needs v0.14.0,
v0.15.0, v0.16.0, v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.14.0 install needs v0.15.0,
v0.16.0, v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.15.0 install needs v0.16.0, v0.17.0, v0.18.0,
v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.16.0 install needs v0.17.0, v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a
v0.17.0 install needs v0.18.0, v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.18.0 install needs
v0.19.0, v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.19.0 install needs v0.20.0, v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.20.0 install needs v0.21.0, v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.21.0 install needs v0.21.1, v0.22.0, v0.22.1, then v0.23.0; a v0.21.1 install needs v0.22.0, v0.22.1, then v0.23.0; a v0.22.0 install needs v0.22.1 then v0.23.0; a v0.22.1 install needs only v0.23.0.
...
NEEDS_V0180_PATCH=no
NEEDS_V0190_PATCH=no
NEEDS_V0200_PATCH=no
NEEDS_V0210_PATCH=no
NEEDS_V0211_PATCH=no
NEEDS_V0220_PATCH=no
...
if [ -d "harness" ]; then
  ...
  NEEDS_V0180_PATCH=yes      # ...then the v0.18.0 map-lane release
  NEEDS_V0190_PATCH=yes      # ...then the v0.19.0 run-lane release
  NEEDS_V0200_PATCH=yes      # ...then the v0.20.0 slice-PR release
  NEEDS_V0210_PATCH=yes      # ...then the v0.21.0 pipeline-speed release
  NEEDS_V0211_PATCH=yes      # ...then the v0.21.1 comment-brevity patch
  NEEDS_V0220_PATCH=yes      # ...then the v0.22.0 pipeline-speed release
  ...
  # v0.18.0: the map-lane release. Keep these markers in sync with
  # migrate-v0.17.0-to-v0.18.0.sh's own idempotency check (platform-scoped
  # wiring markers live in the script — the core file markers suffice for
  # detection here). Any missing ⇒ pre-v0.18.0.
  if [ ! -f espalier/skills/espalier-map/SKILL.md ] \
     ...
     || ! grep -q '^max-open-tickets:' espalier/.espalier-config 2>/dev/null; then
    NEEDS_V0180_PATCH=yes
  fi
  # v0.19.0: the run-lane release. Keep these markers in sync with
  # migrate-v0.18.0-to-v0.19.0.sh's own idempotency check (the script itself
  # rechecks the full engine-file set). Any missing ⇒ pre-v0.19.0.
  if [ ! -f espalier/skills/espalier-maprun/SKILL.md ] \
     || [ ! -f espalier/hooks/maprun.py ] \
     || ! grep -qF '/espalier-maprun' espalier/pipeline.md 2>/dev/null \
     || ! grep -qF 'espalier-maprun' espalier/skills/espalier-map/SKILL.md 2>/dev/null; then
    NEEDS_V0190_PATCH=yes
  fi
  # v0.20.0: the slice-PR release. Keep in sync with
  # migrate-v0.19.0-to-v0.20.0.sh's own idempotency check.
  if [ ! -f espalier/hooks/maprun-pr.sh ] \
     || ! grep -qF 'maprun-pr.sh' espalier/skills/espalier-maprun/SKILL.md 2>/dev/null; then
    NEEDS_V0200_PATCH=yes
  fi
  # v0.21.0: the pipeline-speed release. Keep in sync with
  # migrate-v0.20.0-to-v0.21.0.sh's own idempotency check. An agent marker
  # counts as handled when present OR when the script recorded a customised
  # skip for it in espalier/.migrations-skipped (same E5 rule as v0.10/v0.11:
  # a declined customised file is handled-manual, never re-proposed).
  ok21() { grep -qF "$1" "$2" 2>/dev/null \
           || grep -qF "v0.21.0-$3" espalier/.migrations-skipped 2>/dev/null; }
  if ! grep -qF 'DELTA SCOPE' espalier/pipeline.md 2>/dev/null \
     || ! grep -qF 'Stage 3 Entry: Context Pack' espalier/skills/espalier/SKILL.md 2>/dev/null \
     || ! grep -qF 'context-pack.md' espalier/skills/espalier-fix/SKILL.md 2>/dev/null \
     || ! grep -qF 'pairwise INDEPENDENT' espalier/skills/espalier-grill/SKILL.md 2>/dev/null \
     || ! ok21 'CONTEXT PACK' espalier/agents/harness-coder.md coder-context-pack \
     || ! ok21 'names a CONTEXT PACK' espalier/agents/harness-reviewer.md reviewer-context-pack \
     || ! ok21 'Delta read scope' espalier/agents/harness-reviewer.md reviewer-delta-scope \
     || ! ok21 'names a CONTEXT PACK' espalier/agents/harness-security.md security-context-pack \
     || ! ok21 'Delta mode (when YOUR prior round was clean)' espalier/agents/harness-security.md security-delta-mode; then
    NEEDS_V0210_PATCH=yes
  fi
  # v0.21.1: the comment-brevity patch. Keep in sync with
  # migrate-v0.21.0-to-v0.21.1.sh's own idempotency check; same
  # skip-record-counts-as-handled rule (labels are v0.21.1-*).
  ok211() { grep -qF "$1" "$2" 2>/dev/null \
            || grep -qF "v0.21.1-$3" espalier/.migrations-skipped 2>/dev/null; }
  if ! ok211 'Comments: SHORT' espalier/agents/harness-coder.md coder-comment-brevity \
     || ! ok211 'OVERLONG comment' espalier/agents/harness-reviewer.md reviewer-comment-brevity \
     || ! ok211 'Keep comments SHORT' espalier/rules/coding-standards.md standards-comment-brevity; then
    NEEDS_V0211_PATCH=yes
  fi
  # v0.22.0: the second pipeline-speed release. Keep in sync with
  # migrate-v0.21.1-to-v0.22.0.sh's own idempotency check; same
  # skip-record-counts-as-handled rule (labels are v0.22.0-*).
  ok220() { grep -qF "$1" "$2" 2>/dev/null \
            || grep -qF "v0.22.0-$3" espalier/.migrations-skipped 2>/dev/null; }
  if ! grep -qF 'dispatched SPECULATIVELY' espalier/pipeline.md 2>/dev/null \
     || ! grep -qF 'Stage 4 → Stage 5 handoff' espalier/skills/espalier/SKILL.md 2>/dev/null \
     || ! grep -qF 'coding-report.part-test.md' espalier/skills/espalier-fix/SKILL.md 2>/dev/null \
     || ! grep -qF 'Stage durations' espalier/hooks/espalier-stats.sh 2>/dev/null \
     || ! ok220 'Speculative & Contract entry points' espalier/agents/harness-coder.md coder-speculative \
  ...
  # v0.22.1: the comment-diet patch. Keep in sync with
  # migrate-v0.22.0-to-v0.22.1.sh's own idempotency check; same
  # skip-record-counts-as-handled rule (labels are v0.22.1-*).
  ok2211() { grep -qF "$1" "$2" 2>/dev/null \
             || grep -qF "v0.22.1-$3" espalier/.migrations-skipped 2>/dev/null; }
  if ! ok2211 'the default is NO comment' espalier/agents/harness-coder.md coder-comment-diet \
     || ! ok2211 'Default to NO comment' espalier/rules/coding-standards.md standards-comment-diet \
     || ! ok2211 'EXCESS comments' espalier/agents/harness-reviewer.md reviewer-excess-tag; then
    NEEDS_V0221_PATCH=yes
  fi
  # v0.23.0: the round-economy fold. Keep in sync with
  # migrate-v0.22.1-to-v0.23.0.sh's own idempotency check; same
  # skip-record-counts-as-handled rule (labels are v0.23.0-*).
  ok230() { grep -qF "$1" "$2" 2>/dev/null \
            || grep -qF "v0.23.0-$3" espalier/.migrations-skipped 2>/dev/null; }
  if ! grep -qF 'Contract Phase (folded)' espalier/pipeline.md 2>/dev/null \
     || ! grep -qF 'Stage 5/6 (folded)' espalier/skills/espalier/SKILL.md 2>/dev/null \
     || ! grep -qF 'REGRESSION_VERIFIED_SCOPE' espalier/skills/espalier-fix/SKILL.md 2>/dev/null \
     || ! grep -qF 'mode=score' espalier/skills/espalier-grill/SKILL.md 2>/dev/null \
     || ! grep -qF 'untiered' espalier/hooks/espalier-stats.sh 2>/dev/null \
     || ! ok230 'Contract entry point (post-panel dispatch mode)' espalier/agents/harness-coder.md coder-fold \
     || ! ok230 '## Test Review (folded Stage 4 / serial Stage 6)' espalier/agents/harness-reviewer.md reviewer-fold \
     || ! ok230 'fixture-data leakage' espalier/agents/harness-security.md security-fold \
     || ! ok230 '(- )?Current Stage:' espalier/hooks/pre-push-gate.sh gate-fold; then
    NEEDS_V0230_PATCH=yes
  fi
fi
...
if [ "$NEEDS_V01_V02" = no ] && [ "$NEEDS_V03_V04" = no ] \
   ...
   && [ "$NEEDS_V0160_PATCH" = no ] && [ "$NEEDS_V0170_PATCH" = no ] \
   && [ "$NEEDS_V0180_PATCH" = no ] && [ "$NEEDS_V0190_PATCH" = no ] \
   && [ "$NEEDS_V0200_PATCH" = no ] && [ "$NEEDS_V0210_PATCH" = no ] \
   && [ "$NEEDS_V0211_PATCH" = no ] \
   && [ "$NEEDS_V0220_PATCH" = no ] && [ "$NEEDS_V0221_PATCH" = no ] \
   && [ "$NEEDS_V0230_PATCH" = no ]; then
...
if [ -n "${CLAUDE_SKILL_DIR:-}" ] \
   && [ -f "${CLAUDE_SKILL_DIR}/../../scripts/migrate-v0.22.1-to-v0.23.0.sh" ]; then
...
if [ -z "$PLUGIN_DIR" ]; then
  for candidate in "${ESPALIER_PLUGIN_DIR:-}" "$HOME/repos/espalier-engineering"; do
    ...
    if [ -f "$candidate/scripts/migrate-v0.22.1-to-v0.23.0.sh" ]; then
...
is likely stale (no `migrate-v0.22.1-to-v0.23.0.sh`) — tell the user to
...
[ "$NEEDS_V0180_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.17.0-to-v0.18.0.sh" --dry-run --plugin-dir="$PLUGIN_DIR"
[ "$NEEDS_V0190_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.18.0-to-v0.19.0.sh" --dry-run --plugin-dir="$PLUGIN_DIR"
[ "$NEEDS_V0200_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.19.0-to-v0.20.0.sh" --dry-run --plugin-dir="$PLUGIN_DIR"
[ "$NEEDS_V0210_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.20.0-to-v0.21.0.sh" --dry-run --plugin-dir="$PLUGIN_DIR"
[ "$NEEDS_V0211_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.21.0-to-v0.21.1.sh" --dry-run --plugin-dir="$PLUGIN_DIR"
[ "$NEEDS_V0220_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.21.1-to-v0.22.0.sh" --dry-run --plugin-dir="$PLUGIN_DIR"
...
[ "$NEEDS_V0180_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.17.0-to-v0.18.0.sh" --yes --plugin-dir="$PLUGIN_DIR"
[ "$NEEDS_V0190_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.18.0-to-v0.19.0.sh" --yes --plugin-dir="$PLUGIN_DIR"
[ "$NEEDS_V0200_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.19.0-to-v0.20.0.sh" --yes --plugin-dir="$PLUGIN_DIR"
[ "$NEEDS_V0210_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.20.0-to-v0.21.0.sh" --yes --plugin-dir="$PLUGIN_DIR"
[ "$NEEDS_V0211_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.21.0-to-v0.21.1.sh" --yes --plugin-dir="$PLUGIN_DIR"
[ "$NEEDS_V0220_PATCH" = yes ] && bash "$PLUGIN_DIR/scripts/migrate-v0.21.1-to-v0.22.0.sh" --yes --plugin-dir="$PLUGIN_DIR"
